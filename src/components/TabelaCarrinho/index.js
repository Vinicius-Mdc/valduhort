...
import { BotaoAtualizaTotalItens, ColunaTabela, ColunaTituloTabela, ImagemColunaTabela, LinhaTabela, LinhaTituloTabela, OpcaoColunaTabela, RemoverItem, Tabela, TabelaCabeca, TextoColunaTabela, TotalColunaTabela, TotalItens } from './styles'
...
function TabelaCarrinho({ dados }) {
  const titulos = ['Produto', 'Preço', 'Quantidade', 'Total']
  ...
  return (
    <Tabela cellSpacing={0} cellPadding={0}>
      <TabelaCabeca>
        <LinhaTituloTabela>
          {titulos.map((titulo, id) => (
            <ColunaTituloTabela key={id}>{titulo}</ColunaTituloTabela>
          ))}
          <ColunaTituloTabela key={-1}></ColunaTituloTabela>
        </LinhaTituloTabela>
      </TabelaCabeca>
      {dados.map((item) => (
        <tbody key={item.id}>
          <LinhaTabela key={item.id}>
            <ColunaTabela width={50}>
              <ImagemColunaTabela src={item.foto} />
            </ColunaTabela>
            <ColunaTabela width={60}>
              <TextoColunaTabela>
                {`R$ ${item.preco.toFixed(2).replace('.', ',')}`}
              </TextoColunaTabela>
            </ColunaTabela>
            <ColunaTabela width={120}>
              <TotalColunaTabela>
            ...
            <ColunaTabela width={50}>
              <TotalItens>{`R$ ${(item.total * item.preco)
                .toFixed(2)
                .replace('.', ',')}`}</TotalItens>
            </ColunaTabela>
            <OpcaoColunaTabela>
